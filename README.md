...
### v0.85.00
- Adaptive Runtime GUIを導入
- 設定画面に適応・成長状態パネルを追加
- User Model / Emotion / Personality Growth / Reflection / Important MemoryをGUIから確認可能に変更
- Runtime Counterと重複判定履歴をGUIからリセット可能に変更
- 学習データ本体を誤って削除しない安全なリセット方式を採用


### v0.84.00
- Adaptive Status APIを導入
- personality/adaptive_status_api.py を追加
- /adaptive/statusでUser Model / Emotion / Personality Growth / Reflection / Important Memoryを確認可能に変更
- /adaptive/resetで指定範囲の適応データをリセット可能に変更
- デフォルトではRuntime Stateと重複判定履歴だけをリセット


### v0.83.00
- Adaptive Runtimeの重複更新防止を導入
- personality/adaptive_dedup.py を追加
- 短時間の同一入力・同一出力による重複学習を防止
- 重複時はRuntime Counter / Emotion / Reflectionを更新しない構造へ変更
- 同一内容のImportance Memory重複保存を防止
- adaptive_dedup_state.jsonをGit管理対象外へ変更


### v0.82.00
- Adaptive Runtimeを通常チャットへ接続
- /askの通常会話処理でprocess_adaptive_inputを実行
- LLM回答完成後にprocess_adaptive_outputを実行
- User Model / Important Memory / Emotion / Reflection / Personality Growthを通常会話から自動更新
- Adaptive Runtime失敗時も通常チャットを継続する例外保護を追加
- /ask_streamはaskを利用する既存構造により二重接続を回避


### v0.81.00
- Adaptive Runtime Coordinator基盤を導入
- personality/adaptive_runtime_config.py を追加
- personality/adaptive_event_detector.py を追加
- personality/adaptive_runtime_state.py を追加
- personality/adaptive_runtime_coordinator.py を追加
- Growth / Memory Importance / Emotion / Reflection / Personality Growth / Preference Historyを一括制御可能に変更
- 入力前処理と回答後処理を分離
- 発言回数に応じたReflection・Snapshot・Emotion Decay制御を追加
- 通常チャットへの接続はv0.82以降で実施


### v0.80.00
- Emotion Engine を導入
- personality/emotion_state.py を追加
- personality/emotion_engine.py を追加
- mood / energy / confidence / stress / curiosity / focus / warmthを管理
- 感情イベント更新と時間経過による減衰処理を追加
- 感情状態をPrompt RouterとPrompt Debuggerへ統合


### v0.79.00
- Automatic Personality Growth基盤を導入
- personality/personality_growth_state.py を追加
- personality/personality_growth_manager.py を追加
- 経験値 / 得意分野 / 応答技能 / 自信度を別状態として管理
- 人格の核心を変更せず成長状態だけをPromptへ統合


### v0.78.00
- Conversation Reflection を導入
- personality/reflection_engine.py を追加
- 会話全体から主な話題と回答希望を抽出可能に変更
- 直近の振り返りをPrompt Routerへ統合
- conversation_reflections.jsonをGit管理対象外へ変更
